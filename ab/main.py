...
def download_folders(folders):
    ...
    # 압축 파일을 클라이언트에게 전송
    return send_file(os.path.join(os.getcwd(), temp_zip_path), as_attachment=True, download_name=folders+'.zip')
...
def upload_file(folders):
    ...
    # 파일을 업로드 폴더에 저장
    file.save(os.path.join(os.path.join(os.getcwd(), folders), file.filename))
